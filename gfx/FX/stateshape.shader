...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			OutColor.rgb = gray*0.5f;
			//float3 hsv = RGBtoHSV(OutColor.rgb);
			//float3 rgb = HSVtoRGBPost(hsv);
			//OutColor.rgb = rgb;
		#ifdef ANIMATED
			OutColor = Animate(OutColor, v.vTexCoord, v.vAnimatedTexCoord, MaskTexture, AnimatedTexture, MaskTexture2, AnimatedTexture2);
		#endif
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			vTime *= vTime;
			vTime = 0.9*0.9 - vTime;
		    float4 MixColor = float4( 0.15, 0.15, 0.15, 1.0 ) * vTime;
			if(!all(OutColor.rgb) && !(OutColor.a == 0)) {
				OutColor.rgb = float3( 0.95, 0.93, 0.565);
				OutColor.a = 1.0;
			};
		    OutColor.rgb += ( 0.5 + OutColor.rgb ) * MixColor.rgb;
